...
interface PricingDetailData {
  ...
  profit: number;
  createdAt: string;
  conversation: PricingDetailMessage[];
}

interface PricingDetailMessage {
  id: string;
  message: string;
  sender: 'user' | 'ia';
  timestamp: string;
...
const MOCK_PRICING_DETAIL: PricingDetailData = {
  id: 'pricing-001',
  clinicId: '1',
  clinicName: 'Clínica OdontoSoberana',
...
export const PricingDetail: React.FC = () => {
  ...
  const [pricing, setPricing] = useState<PricingDetailData>(
    MOCK_PRICING_DETAIL,
  );
  ...
  useEffect(() => {
    ...
    const loadPricing = async () => {
      ...
      try {
        if (!id) {
          if (isMounted) {
            setPricing(MOCK_PRICING_DETAIL);
          }

          return;
        }

        const data = await pricingService.getPricing(id);

        if (!isMounted) {
          return;
        }

        const apiData = data as Partial<PricingDetailData>;
        ...
        setPricing({
          ...
          id: apiData.id ?? id,
          conversation:
            apiData.conversation ?? MOCK_PRICING_DETAIL.conversation,
        });
      } catch {
        if (isMounted) {
          setPricing({
            ...MOCK_PRICING_DETAIL,
            id: id ?? MOCK_PRICING_DETAIL.id,
          });
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const pdf = await pricingService.exportPDF(pricing.id);

      const url = URL.createObjectURL(pdf);
      const link = document.createElement('a');

      link.href = url;
      link.download = `precificacao-${pricing.id}.pdf`;
    ...
    return (
      <main className="pricing-detail">
        <div className="pricing-detail__loading">
          ...
          <span>Carregando precificação...</span>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="pricing-detail">
      <div className="pricing-detail__container">
        ...
        {error && (
          <div className="pricing-detail__notice" role="status">
            {error}
          </div>
        )}
        ...
        <section className="pricing-detail__info-card">
          <div className="pricing-detail__section-header">
            <h2>Informações da Precificação</h2>
          </div>

          <div className="pricing-detail__info-grid">
            <div className="pricing-detail__info-item">
              <span>Clínica</span>
              <strong>{pricing.clinicName}</strong>
            </div>

            <div className="pricing-detail__info-item">
              <span>Procedimento</span>
              <strong>{pricing.procedureName}</strong>
            </div>

            <div className="pricing-detail__info-item">
              <span>Data da precificação</span>
              <strong>{formatDate(pricing.createdAt)}</strong>
            </div>

            <div className="pricing-detail__info-item">
              <span>ID da precificação</span>
              <strong>{pricing.id}</strong>
            </div>
          </div>
        </section>
        ...
        <section className="pricing-detail__conversation-card">
          <div className="pricing-detail__section-header">
            <div>
              <h2>Conversa com a Soberana AI</h2>
              <p>
                Histórico completo da conversa utilizada na
                precificação.
              </p>
            </div>
          </div>
          ...
          <div className="pricing-detail__timeline">
            {pricing.conversation.map((message) => (
              <div
                key={message.id}
        ...
        <section className="pricing-detail__result-card">
          <div className="pricing-detail__section-header">
            <div>
              ...
              <p>
                Detalhamento dos principais valores da
                precificação.
              </p>
            </div>
          </div>
          ...
          <div className="pricing-detail__result-grid">
            ...
            <div className="pricing-detail__result-item">
              ...
              <strong>
                {formatCurrency(pricing.fixedCosts)}
              </strong>
            </div>
            ...
            <div className="pricing-detail__result-item">
              ...
              <strong>
                {formatCurrency(pricing.variableCosts)}
              </strong>
            </div>
            ...
            <div className="pricing-detail__result-item">
              ...
              <strong>
                {formatCurrency(pricing.hourCost)}
              </strong>
            </div>
            ...
            <div className="pricing-detail__result-item">
              <span>Comissão</span>
              <strong>
                {formatPercentage(pricing.commission)}
              </strong>
            </div>
            ...
            <div className="pricing-detail__result-item">
              <span>Impostos</span>
              <strong>
                {formatPercentage(pricing.taxes)}
              </strong>
            </div>
